...
class designController {
	create_user_design = async (req, res) => {
		...
		try {
			...
			const design = await designModel.create({
				institute_id: _id,
				templateTitle: JSON.parse(fields.design[0]).templateTitle,
	...
	get_user_designs = async (req, res) => {
		...
		try {
			const designs = await designModel
				.find({ institute_id: new ObjectId(_id) })
				.sort({ createdAt: -1 });
	...
	delete_user_design = async (req, res) => {
		...
		try {
			...
			return res
				.status(200)
				.json({ message: "Design deleted successfully" });
		} catch (error) {
			return res.status(500).json({ message: error.message });
		}
	};
	...
	add_background_image = async (req, res) => {
		...
		try {
			...
			const userImage = await backgroundImageModel.create({
				institute_id: _id,
				image_url: url,
			});
			return res.status(201).json({ userImage });
